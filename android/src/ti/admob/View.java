...
public class View extends TiUIView implements AdListener
{
	...
	private Map<String, Object> createAdRequestProperties(){		
		...
		if(prop_color_bg_top!=null) extras.put("color_bg_top", prop_color_bg_top);
		if(prop_color_border!=null) extras.put("color_border", prop_color_border);
		if(prop_color_text!=null) extras.put("color_text", prop_color_text);
		if(prop_color_link!=null) extras.put("color_link", prop_color_link);
		if(prop_color_url!=null) extras.put("color_url", prop_color_url);
		return extras;		
	}
	// modifies the color prop -- removes # and changes constants into hex values
	private String convertColorProp(String color){
		color = color.replace("#", "");
		if(color.equals("white")) color="FFFFFF";
		if(color.equals("red")) color = "FF0000";
		if(color.equals("blue")) color="0000FF";
		if(color.equals("green")) color="008000";
		if(color.equals("yellow")) color="FFFF00";
		if(color.equals("black")) color="000000";
		return color;
	}
